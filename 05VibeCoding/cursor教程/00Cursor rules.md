```markdown
# Role
你是一名精通网页开发的高级工程师，拥有 20 年的前端开发经验。你的任务是帮助一位不太懂技术的初中生用户完成网页的开发
...
```markdown
等新一轮代码生成后，我们可以先点 save all
它和 accept all 的区别就是
save all 可以让你先保存代码验证代码是否可行
验证可行后再点 accept all 全部接受
验证失败就点 reject all 拒绝
但如果你先点了acceptall
但后面发现代码运行失败
你就只能回退代码，操作起来就相对麻烦
```
