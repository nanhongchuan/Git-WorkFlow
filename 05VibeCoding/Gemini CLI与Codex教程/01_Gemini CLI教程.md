...
**视频地址**：https://www.bilibili.com/video/BV1LL3LzmE36
...
#### 1\. 情况一：网络原因导致的认证失败
  ...
  * **临时解决（当前终端）**：
    ...
    3.  重新启动 `gemini` 进行认证。
  * **全局生效（所有终端）**：
  1.  **方法一：** 开启代理软件的**增强模式/TUN 模式**。

  >  TUN 模式 是一种通过虚拟网卡让 Clash 接管系统所有网络流量 的模式。它的作用是：**让不支持代理的程序（比如某些游戏、命令行工具、系统服务）也能走 Clash 代理。**
  
  2.  **方法二：** 将上述 `export https_proxy= http_proxy= all_proxy=` 命令添加到您的系统配置文件中（如 **`~/.zshrc`** 或 **`~/.bashrc`**）。例如使用 `vim` 编辑对 `.zshrc` 文件进行编辑，然后添加 export 命令设置环境变量：  
...
#### 2\. 情况二：账号配置问题（需设置 Project ID）
  ...
  * **1️⃣临时解决（当前终端）**：
    ...
    1.  获取您的 Google Cloud **项目 ID**（Project ID）。
      ...
      - 获取GOOGLE_CLOUD_PROJECT ID
      https://console.cloud.google.com/welcome?project=
      
      she- 启用Google Cloud GEMINI的 API
      https://console.developers.google.com/apis/api/cloudaicompanion.googleapis.com/overview?project=
...
#### 3\. 进入Gemini CLI
  * 若未进入Gemini CLI，那么输入`gemini`可以进行认证；
  * 若已经进入可以输入`/auth`可以再次进行认证
...
### 五、如何在 Gemini CLI 中切换模型

在 Gemini CLI 中切换模型有几种方法，可以在启动时指定，也可以在交互会话中使用命令切换。

#### 1. 使用命令行标志 (启动时指定)

启动 Gemini CLI 时，可以使用 `--model` 或 `-m` 标志来指定要使用的模型。这对单次会话特别有用。

例如：

* 使用 `gemini-2.5-flash` 模型（速度更快）：
    ```bash
    gemini --model gemini-2.5-flash
    ```
    > 请谨慎使用此类代码。

* 使用 `gemini-2.5-pro` 模型（功能更强大，默认模型）：
    ```bash
...
进入交互模式后，可以使用 `/model` 命令动态切换模型。

1.  运行 `gemini` 进入交互模式。
2.  在提示符下输入 `/model`。
3.  系统将打开一个对话框或菜单，允许选择一个模型，例如 `Auto`（自动选择）、`Pro`、`Flash` 或 `Flash-Lite`。

#### 3. 设置环境变量 (永久设置默认模型)

如果希望默认使用特定的模型，可以设置 `GEMINI_MODEL` 环境变量。这样，每次运行 `gemini` 命令时都会使用该模型，除非使用命令行标志覆盖它。

* **对于 Linux/macOS** (在 `.bashrc`, `.zshrc` 或类似文件中)：
    ```bash
...
设置完环境变量后，请重启终端以使更改生效。

#### 可用模型

常用的模型包括:

* **Auto (推荐)**: 让系统为任务选择最佳模型。
* **Pro**: 适用于需要深度推理和创造力的复杂任务（默认模型）。
* **Flash**: 兼顾速度和推理能力，响应通常更快。
* **Flash-Lite**: 适用于需要快速完成的简单任务。



### 六、如何在 Gemini CLI 中更新

### 💻 手动更新命令

请在您的终端（Terminal）或命令行中运行以下命令来更新到最新版本：
...
#### 命令解释：
  ...
  * `npm install`：这是 `npm` 的安装命令（也用于更新）。
  * `-g`：是 `global`（全局）的缩写，因为 CLI（命令行工具）通常是全局安装的。
  * `@google/gemini-cli@latest`：这是 Gemini CLI 的官方包名，`@latest` 标签会确保您获取最新的稳定版本。

-----

### 🤔 备选命令

您也可以尝试使用 `npm` 的 `update` 命令：
...
更新完成后，您可以运行以下命令来检查当前安装的版本，确认它是否显示为最新的 `0.15.1` 或更高版本：
...
> **注意：**
> 如果您在 macOS 或 Linux 上运行时遇到“权限不足”（Permission denied）的错误，您可能需要在命令前加上 `sudo`：
>
> ```bash
