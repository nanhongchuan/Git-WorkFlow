import json
import os
import pandas as pd
from langchain_openai import ChatOpenAI
from langchain_experimental.agents import create_pandas_dataframe_agent
from dotenv import load_dotenv


PROMPT_TEMPLATE = """
1. 对于文字回答的问题，按照这样的格式回答：
   {"answer": "<你的答案写在这里>"}
例如:
   {"answer": "订单量最高的ID是_MNWC3-067"}

2. 如果用户需要一个表格，按照这样的格式回答:
   {"table": {"columns": ["column1", "column2", ...], "data": [[value1, value2, ...], [value1, value2,  ...]}}

3. 如果用户的请求适合返回条形图，按照这样的格式回答:
   {"bar": {"columns": ["Category", "Value"], "data": [["A", 34], ["B", 21], ["C", 91]]}}

4. 如果用户的请求适合返回折线图，按照这样的格式回答:
   {"line": {"columns": ["Category", "Value"], "data": [["A", 34], ["B", 21], ["C", 91]]}}

5. 如果用户的请求适合返回散点图，按照这样的格式回答:
   {"scatter": {"columns": ["Category", "Value"], "data": [["A", 34], ["B", 21], ["C", 91]]}}

注意: 我们只支持三种类型的图表: "bar", "line" 和 "scatter"。

请将所有输出作为JSON字符串返回。请注意要将"columns"列表和数据列表中的所有字符串都用双引号包围。
例如: {"columns": ["Products", "Orders"], "data": [["32085Lip", 245], ["76439Eye", 178]]}
"""

# "data" 的值是一个嵌套列表（list of lists）， "columns" 的值是一个字符串列表

load_dotenv()

def dataframe_agent(openai_api_key, df, query):   
    # 从环境变量读取配置
    api_base = os.getenv("OPENAI_API_BASE") 
    # 创建模型实例
    model = ChatOpenAI(
        model="gemini-2.5-flash-preview-05-20",
        openai_api_key=openai_api_key,
        base_url=api_base,
        temperature=0
    )
    agent = create_pandas_dataframe_agent(llm=model,
                                          df=df, 
                                          agent_executor_kwargs={"handle_parsing_errors":True},
                                          verbose=True,
                                          allow_dangerous_code=True)
    prompt = PROMPT_TEMPLATE + query
    response = agent.invoke({"input": prompt})
    response_dict = json.loads(response["output"])
    return response_dict


# df = pd.read_csv("personal_data.csv")
# print(dataframe_agent(os.getenv(OpenAI_API_Key),df, "数据里出现最多的职业是什么？")) 